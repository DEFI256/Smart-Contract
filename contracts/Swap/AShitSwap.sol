...
import "../LPtoken/AShitLPToken.sol"; // 修改为 AShitLPToken

contract AShitSwap is Ownable {
    IERC20 public tokenA; // 保持 tokenA 不变
    IERC20 public Shit; // 改为 Shit
    AShitLPToken public lpToken; // 修改为 AShitLPToken

    uint256 public priceCurrent;
    
    uint256 public reserveA; // 保持 reserveA 不变
    uint256 public reserveShit; // 改为 reserveShit
    ...
    event SwapExecuted(
        ...
        bool isAToShit, // 改为 isAToShit
        uint256 reserveA, // 保持 reserveA 不变
        uint256 reserveShit, // 改为 reserveShit
        uint256 amountIn,
        uint256 amountOut
    );
    ...
    constructor(address _tokenA, address _Shit, address _lpToken) Ownable(msg.sender) {
        tokenA = IERC20(_tokenA);
        Shit = IERC20(_Shit);
        lpToken = AShitLPToken(_lpToken); // 修改为 AShitLPToken
    }

    /** @dev 添加流动性 */
    function addLiquidity(uint256 amountA, uint256 amountShit) external {
        require(amountA > 0 && amountShit > 0, "Amounts must be greater than zero");
        ...
        uint256 lpAmount;
        if (totalLiquidity == 0) {
            lpAmount = (amountA + amountShit) / 2;
            priceCurrent = amountA / amountShit;
        } else {
    ...
    /** @dev 计算交易详情 */
    function getSwapDetails(uint256 amountIn, bool isAToShit) 
        public
        returns (uint256 feeAmount, uint256 estimatedAmountOut, uint256 priceImpact) 
    {
        require(amountIn > 0, "Amount must be greater than zero");

        uint256 x = isAToShit ? reserveA : reserveShit;
        uint256 y = isAToShit ? reserveShit : reserveA;
    ...
    /** @dev 兑换稳定币，支持滑点保护 */
    function swap(uint256 amountIn, bool isAToShit, uint256 minAmountOut) external {
        require(amountIn > 0, "Amount must be greater than zero");

        (, uint256 amountOut, ) = getSwapDetails(amountIn, isAToShit);
        ...
        priceCurrent = reserveA / reserveShit;

        emit SwapExecuted(block.timestamp, priceCurrent, isAToShit, reserveA, reserveShit, amountIn, amountOut);
    }
        ...
        returns (uint256 pairedAmount) 
    {
        ...
        if (isA) {
            pairedAmount = (amountIn * reserveShit) / reserveA;
        } else {
            pairedAmount = (amountIn * reserveA) / reserveShit;
        }
    }

    function getReservesAndLiquidity() public view 
        returns (uint256 reserveA_, uint256 reserveShit_, uint256 totalLiquidity_, uint256 priceCurrent_) 
    {
        reserveA_ = reserveA;
